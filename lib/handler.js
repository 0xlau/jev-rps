  ...
  return async function POST(request) {
    try {
      ...
      if (request.headers.get('sec-fetch-site') === 'cross-site' || (origin && new URL(origin).host !== host)) {
        return send(403, { error: '不接受其他网站发起的请求。' });
      }
      if (!request.headers.get('content-type')?.includes('application/json')) return send(415, { error: '请求必须为 JSON。' });
      if (Number(request.headers.get('content-length')) > 4_000_000) return send(413, { error: '请求过大。' });
      const reader = request.body?.getReader();
      if (!reader) return send(400, { error: '请求不能为空。' });
      const chunks = []; let size = 0;
      while (true) {
        const { done, value } = await reader.read();
        if (done) break;
        size += value.byteLength;
        if (size > 4_000_000) { await reader.cancel(); return send(413, { error: '请求过大。' }); }
        chunks.push(value);
      }
      let data;
      try { data = JSON.parse(Buffer.concat(chunks).toString('utf8')); }
      catch { return send(400, { error: 'JSON 格式无效。' }); }
      if (!data || Array.isArray(data) || typeof data !== 'object') return send(400, { error: '请求格式无效。' });
      if (!['prepare', 'reveal', 'resolve'].includes(data.action)) return send(400, { error: '未知操作。' });
      ...
      return send(500, { code: 'INTERNAL_ERROR', error: '本次操作未完成，请重试。已锁定的出拳会保留。' });
    }
  };
}
