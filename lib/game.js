...
export function createGameService({ secret, fetchImpl = fetch, clock = () => new Date().toISOString() }) {
  if (typeof secret !== 'string' || secret.length < 32) {
    fail(503, 'SETUP_REQUIRED', 'Server proof secret is not configured. Contact the site maintainer.');
  }
  ...
  function unseal(token) {
    try {
      ...
      return value;
    } catch { fail(400, 'INVALID_ROUND', 'This round proof is invalid or expired and cannot be changed or regenerated. Export your record, then start a new series.'); }
  }

  function verifyHistory(history, series) {
    if (!Array.isArray(history) || history.length > MAX_ROUNDS) fail(400, 'INVALID_HISTORY', 'Match record is malformed or over the per-series cap.');
    let previous = null;
    const ids = new Set();
    for (const [index, item] of history.entries()) {
      ...
      if (!r || typeof item.signature !== 'string' || !/^[a-f0-9]{64}$/.test(item.signature)
        ...
        fail(400, 'INVALID_HISTORY', 'Match record failed verification (edited or missing a round). Export first, then start a new series.');
      }
  ...
  async function prepare({ history, series, apiKey }) {
    if (!isId(series)) fail(400, 'INVALID_SERIES', 'Series id is invalid. Refresh the page.');
    if (typeof apiKey !== 'string' || apiKey.length < 8 || apiKey.length > 512 || /[^\x21-\x7e]/.test(apiKey)) {
      fail(401, 'MISSING_KEY', 'Enter a valid TypeSafe API Key to begin.');
    }
    const previous = verifyHistory(history, series);
    if (history.length >= MAX_ROUNDS) fail(400, 'SERIES_COMPLETE', 'This series hit 500 rounds. Start a new series — old matches stay local and exportable.');
    ...
    try {
      ...
      fail(502, 'INVALID_ANALYSIS', 'Jev returned no valid behavior analysis. Round did not start — retry.');
    }
    ...
    if (answer?.type !== 'choice' || !MOVES.includes(answer.choice) || typeof result.model !== 'string'
      ...
      fail(502, 'INVALID_ANSWER', 'Jev returned no valid rock, paper, or scissors. Round did not start — retry.');
    }
    ...
    const strategy = { ...analysis, blindRounds: evidence.blind_rounds,
      ...
      selfPredictability: evidence.jev_self_predictability, exploitationRate: evidence.human_exploitation_rate,
      exploitAlert: evidence.exploit_alert };
  ...
  function resolve({ sealedRound, player, peeked = false }) {
    if (!MOVES.includes(player) || typeof peeked !== 'boolean') fail(400, 'INVALID_MOVE', 'Choose rock, paper, or scissors.');
    const decision = unseal(sealedRound);
